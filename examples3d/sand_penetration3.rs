...
pub fn init_world(testbed: &mut Testbed) {
    ...
    for i in 0..=nx {
        for j in 0..=ny {
            heigths[(i, j)] = (i as f32 * std::f32::consts::PI / (nx as f32) * 10.0).sin();
        }
    }
    colliders.insert(
        ColliderBuilder::heightfield(
            heigths.clone().into(),
            vector![ground_half_side * 2.0, 1.0, ground_half_side * 2.0],
        )
    ...
    colliders.insert(
        ...
        .translation(vector![0.0, 12.0, 0.0])
        .rotation(vector![std::f32::consts::PI, 0.0, 0.0])
        .build(),
    );
    ...
    colliders.insert(
        ...
        .rotation(vector![-std::f32::consts::PI / 2.0, 0.0, 0.0])
        .build(),
    );
    ...
    let sand_particles = helper::cube_particles(
        point![
            0.0,
            2.0 * (cell_width * 3.0 + 2.0 + particle_rad * 2.0 * 50.0),
            0.0
        ],
        100, // 40,
        50,  // 100,
